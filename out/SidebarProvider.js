...
class SidebarProvider {
    ...
    workspaceVersion = 0;
    ollamaAbortControllers = new Map();
    childProcesses = [];
    ...
    static MODEL_PROVIDER_KEY = "vico.selectedProvider";
    ...
    static API_BASE_URL = "http://103.250.10.249:13100/api";
                ...
                try {
                    ...
                    if (message.filePath) {
                        ...
                        if (workspaceFolders) {
                            ...
                            if (analysis.pageBasePath &&
                                !cleanPath.startsWith(analysis.pageBasePath)) {
                                // Only warn for page-related files (page.tsx, layout.tsx, etc.)
                                const isPageFile = cleanPath.includes("page.tsx") ||
                                    cleanPath.includes("layout.tsx") ||
                                    cleanPath.includes("route.ts") ||
                                    cleanPath.includes("page.jsx") ||
                                    cleanPath.includes("layout.jsx");
                                const isWrongPagesDir = cleanPath.startsWith("pages/") ||
                                    cleanPath.startsWith("src/pages/");
                                if (isPageFile && isWrongPagesDir) {
                                    const action = await vscode.window.showWarningMessage(`Warning: Agent is trying to write to '${cleanPath}', but the project uses '${analysis.pageBasePath}'. This might create duplicate pages. Proceed?`, "Proceed", "Cancel");
                                    if (action !== "Proceed")
                                        return;
                                }
                            }
                    ...
                    if (document) {
                        ...
                        if (!message.filePath && selection && !selection.isEmpty) {
                            ...
                            const isSnippetLikely = originalText.length > 1000 &&
                                newText.length < originalText.length * 0.5 &&
                                !newText.includes("import ") &&
                                !newText.includes("export ");
                            if (isSnippetLikely) {
                                const action = await vscode.window.showWarningMessage("The code you are applying looks like a partial snippet but no text is selected. Overwrite the entire file anyway?", "Overwrite", "Cancel");
                                if (action !== "Overwrite") {
                                    return;
                                }
                            }
                        ...
                        const result = await diffManager.openDiff(document.uri, newText);
                ...
                case "ollamaAgent": {
                    ...
                    try {
                        ...
                        const sessionId = data.sessionId ||
                            this.context.globalState.get("currentSessionId");
                        let agentState = null;
                        if (sessionId) {
...
`;
                        ...
                        if (mode === "execute") {
                            const execDirectives = this.buildExecutionDirectives(data.step, data.context || compactContext);
                            if (execDirectives) {
                            ...
                            for (const att of data.attachments) {
                                ...
                                if (content &&
                                    (att.type?.startsWith("text/") ||
                                        att.name?.match(/\.(ts|js|tsx|jsx|json|html|css|md|py|txt)$/i))) {
                                    let text = content;
...
`;
                        ...
                        if (mode === "plan") {
                            ...
                            try {
                                ...
                                const normalized = this.normalizePlanObject(planObj, data.message);
                            ...
                            const normalizedContent = content.replace(/\[(\/?)writeFileVico\s*\]/gi, "[$1writeFile]");
                            webviewView.webview.postMessage({
                            ...
                            // Normalize [writeFileVico] to [writeFile] for consistent processing
                            let normalizedAssistantMessage = assistantMessage.replace(/\[(\/?)writeFileVico\s*\]/gi, "[$1writeFile]");
                            // Clean up hallucinated placement comments from local models
                            normalizedAssistantMessage = normalizedAssistantMessage
                                .replace(/\((Top|End|Bottom|Beginning)\s+of\s+the\s+file\)/gi, "")
                                .replace(/\(at\s+the\s+(Top|End|Bottom|Beginning)\s+of\s+the\s+file\)/gi, "")
                                .replace(/\/\/\s+(Top|End|Bottom|Beginning)\s+of\s+the\s+file/gi, "");
                            webviewView.webview.postMessage({
                                command: "ollamaAgentResponse",
                                mode,
                ...
                case "ollamaChat": {
                    ...
                    try {
                        ...
                        // Normalize [writeFileVico] to [writeFile] for consistent processing
                        const normalizedAssistantMessage = assistantMessage.replace(/\[(\/?)writeFileVico\s*\]/gi, "[$1writeFile]");
                        webviewView.webview.postMessage({
                ...
                case "getAiSettings": {
                    ...
                    const selectedProvider = this.context.globalState.get(SidebarProvider.MODEL_PROVIDER_KEY, "openai");
                    ...
                    const customProviderSettings = (0, aiProviderSettings_1.normalizeCustomProviderSettings)(this.context.globalState.get(aiProviderSettings_1.CUSTOM_PROVIDER_SETTINGS_KEY));
                ...
                case "saveAiSettings": {
                    ...
                    const selectedProvider = (0, aiProviderSettings_1.resolveProviderKind)(String(message.provider || ""), selectedModel);
                    ...
                    await this.context.globalState.update(SidebarProvider.MODEL_PROVIDER_KEY, selectedProvider);
                    await this.context.globalState.update("vico.customApiUrl", customApiUrl);
                    await this.context.globalState.update(aiProviderSettings_1.CUSTOM_PROVIDER_SETTINGS_KEY, customProviderSettings);
                ...
                case "consumePromptQuota": {
                    ...
                    const selectedProvider = (0, aiProviderSettings_1.resolveProviderKind)(String(message.provider ||
                        this.context.globalState.get(SidebarProvider.MODEL_PROVIDER_KEY, "openai")), selectedModel);
                    const customProviderSettings = (0, aiProviderSettings_1.normalizeCustomProviderSettings)(this.context.globalState.get(aiProviderSettings_1.CUSTOM_PROVIDER_SETTINGS_KEY));
                ...
                case "search":
                    if (this.checkDuplicateAction("search", message.query)) {
                        webviewView.webview.postMessage({
                            ...
                            results: "⚠️ You recently performed this search. Please use the previous results or try a different query to avoid looping.",
                        });
                        return;
                    }
                    try {
                        ...
                        const parsedQuery = (0, codeSearch_1.parseSearchQuery)(query || "");
                        if (!query || (0, codeSearch_1.shouldRejectSearchQuery)(parsedQuery)) {
                            webviewView.webview.postMessage({
                                command: "searchResult",
                                results: "Query too short for content search. Use at least 3 literal characters, or prefix with re: for regex search.",
                            });
                            return;
                        }
                        const rankedResults = [];
                            ...
                            try {
                                ...
                                for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
                                    ...
                                    if (regex.test(line)) {
                                        ...
                                        const preview = (0, codeSearch_1.createSearchResultPreview)(line);
                                        ...
                                        if (!seenResults.has(resultStr)) {
                                            ...
                                            rankedResults.push({
                                                ...
                                                lineNum,
                                                preview,
                                                score: (0, codeSearch_1.rankSearchMatch)({
                                                    relativePath,
                                                    lineNum,
                                                    line,
                        ...
                        this.sendLog(`[Search] Completed. Found ${rankedResults.length} matches.`);
                        ...
                        const outputLines = (0, codeSearch_1.formatRankedSearchResults)(rankedResults);
                        const resultPrefix = parsedQuery.warnings.length > 0
                ...
                case "searchSymbols":
                    if (this.checkDuplicateAction("searchSymbols", message.query)) {
                        webviewView.webview.postMessage({
                            ...
                            results: "You recently performed this symbol search. Please use the previous results or try a different query to avoid looping.",
                        });
                        return;
                    }
                    try {
                        ...
                        if (query.length <= 1) {
                            webviewView.webview.postMessage({
                                command: "searchSymbolsResult",
                                results: "Query too short for symbol search. Use at least 2 characters.",
                            });
                            return;
                        }
                        ...
                        const rankedSymbols = symbols
                            ...
                            return {
                                name: symbol.name,
                                kind: (0, codeSearch_1.formatSymbolKind)(symbol.kind),
                                relativePath,
                                lineNum: symbol.location.range.start.line + 1,
                                score: (0, codeSearch_1.rankSymbolMatch)({
                                    symbolName: symbol.name,
                                    symbolKind: (0, codeSearch_1.formatSymbolKind)(symbol.kind),
                                    relativePath,
                                    lineNum: symbol.location.range.start.line + 1,
                            ...
                            .filter((symbol) => !this.isTransientVicoArtifactPath(symbol.relativePath))
                            .sort((a, b) => {
                            if (b.score !== a.score)
                                return b.score - a.score;
                            if (a.relativePath !== b.relativePath) {
                            ...
                            .slice(0, 100);
                        const outputLines = (0, codeSearch_1.formatRankedSymbolResults)(rankedSymbols);
                        webviewView.webview.postMessage({
                            command: "searchSymbolsResult",
                            results: outputLines.length > 0
                                ? outputLines.join("\n")
                                : "No symbols found.",
                        });
                    }
                ...
                case "executeCommand":
                    ...
                    if (/\bls\s+-R\b/.test(message.command)) {
                        ...
                        this.sendLog(`[System] Optimized 'ls -R' with exclusions: ${ignoreFlags}`);
                    }
    ...
    maxTokens = 8000;
    targetSize = 30000; // approximately 7.5k tokens (1 token ≈ 4 characters)
    maxFilesPerFolder = 2;
    normalizeStepType(rawType) {
        const t = String(rawType || "")
            .trim()
            .toLowerCase();
        if (t === "terminal_command" || t === "terminal")
            return "terminal_command";
        if (t === "command" || t === "run_command")
            return "terminal_command";
        if (t === "file_write" || t === "write" || t === "diff")
            return "file_write";
        if (t === "file_read" || t === "read")
            return "file_read";
        if (t === "complete" || t === "done")
            return "complete";
        return "file_write";
    }
    normalizePlanObject(planObj, briefRequest = "task") {
        const normalized = planObj && typeof planObj === "object" ? { ...planObj } : {};
    ...
    buildExecutionDirectives(step, context) {
        ...
        const selfEvalDirective = stepType === "file_write"
            ? `
...
`;
        const outputContractDirective = stepType === "file_write"
            ? [
    ...
    buildLoopBreakerContext(state, stepDescription) {
        if (!state || !Array.isArray(state.recent_steps) || !stepDescription)
            return "";
        const desc = String(stepDescription).trim().toLowerCase();
        if (!desc)
            return "";
        const hits = state.recent_steps.slice(-6).filter((s) => String(s?.description || "")
            .trim()
            .toLowerCase() === desc).length;
        if (hits < 2)
            return "";
    ...
    buildVerificationModeDirective(mode) {
        if (mode === "minimal") {
            return [
    ...
    buildGuardrailsContext(guardrails) {
        ...
        if (whitelist.length > 0) {
            segments.push(`WHITELISTED FILES (preferred targets): ${JSON.stringify(whitelist)}\n` +
        ...
        const limitEntries = lineLimits && typeof lineLimits === "object"
            ? Object.entries(lineLimits).filter(([, limit]) => limit != null)
            : [];
        if (limitEntries.length > 0) {
            const formatted = limitEntries.map(([file, limit]) => `- ${file}: lines ${JSON.stringify(limit)}`);
            segments.push(`LINE LIMITS (apply to the listed files): \n${formatted.join("\n")}\n` +
    ...
    compressContext(context, maxChars = 50000) {
    ...
    async analyzeFramework(rootPath) {
        ...
        try {
            ...
            const hasPages = fs.existsSync(path.join(rootPath, "pages")) ||
...
`;
    ...
    getHtmlForWebview(webview) {
        ...
        const webviewHelpersPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-helpers.js")));
        const webviewWorkflowPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-workflow.js")));
        const webviewDiscoveryPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-discovery.js")));
        const webviewExecutionPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-execution.js")));
        const webviewThinkPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-think.js")));
        const webviewExecuteLoopPath = webview.asWebviewUri(vscode.Uri.file(path.join(this._extensionUri.fsPath, "media", "webview-execute-loop.js")));
        ...
        htmlContent = htmlContent.replace("%WEBVIEW_HELPERS_PATH%", webviewHelpersPath.toString());
        htmlContent = htmlContent.replace("%WEBVIEW_WORKFLOW_PATH%", webviewWorkflowPath.toString());
        htmlContent = htmlContent.replace("%WEBVIEW_DISCOVERY_PATH%", webviewDiscoveryPath.toString());
        htmlContent = htmlContent.replace("%WEBVIEW_EXECUTION_PATH%", webviewExecutionPath.toString());
        htmlContent = htmlContent.replace("%WEBVIEW_THINK_PATH%", webviewThinkPath.toString());
        htmlContent = htmlContent.replace("%WEBVIEW_EXECUTE_LOOP_PATH%", webviewExecuteLoopPath.toString());
